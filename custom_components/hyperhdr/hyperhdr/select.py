...
class HyperHDRColorEngineSelect(HyperHDRSelect):
    ...
    Requires a HyperHDR version that exposes ``colorEngine`` in the
    serverinfo response.  If the server does not support it, the entity
    marks itself as unavailable.
    """
    ...
    @property
    def available(self) -> bool:
        """Return availability — requires server support for color engine."""
        if not self._client.has_loaded_state:
            return False
        # If the server doesn't expose colorEngine data the feature is
        # unsupported and the entity should be unavailable.
        return self._client.color_engine is not None

    async def async_added_to_hass(self) -> None:
        """Register callbacks and populate initial state."""
        await super().async_added_to_hass()
        self._update_color_engine()
    ...
    def _update_color_engine(self, _: dict[str, Any] | None = None) -> None:
        ...
        if hasattr(self._client, "color_engine") and self._client.color_engine:
            ...
            if isinstance(engine_data, dict):
                ...
                if engine_type and engine_type.lower() in self._attr_options:
